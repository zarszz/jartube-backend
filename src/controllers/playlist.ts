...
export async function createPlaylist(req: Request, res: Response): Promise<void> {
    ...
    form.parse(req, async (err: Error, fields: Fields, _: Files): Promise<Response> => {        
        try {
...
export async function updatePlaylist(req: Request, res: Response): Promise<void> {
    ...
    form.parse(req, async (err: Error, fields: Fields, _: Files): Promise<Response> => {        
        try {
            ...
            const data = <IPlaylistDocument> await PlaylistModel.findOneAndUpdate({ _id: id }, playlist, { useFindAndModify: false, new: true });
...
export async function deleteVideoFromPlaylist(req: Request, res: Response) {
    try {
        const { id, videoID } = req.params;
        ...
        playlist.videos = playlist.videos.filter((curr) => { return curr !== videoID });
        playlist.save();
